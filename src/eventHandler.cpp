...
namespace KBX {
  ...
  */
  EventHandler::EventHandler(Scene* scene){
      this->scene = scene;
  }
  
  /// constructor
  ExitEventHandler::ExitEventHandler(Scene* scene) :
      EventHandler(scene)
  {}
  /// handle exit events
  /**
  ...
  */
  int ExitEventHandler::handle(SDL_Event* event){
  ...
  /// constructor
  MotionEventHandler::MotionEventHandler(Scene* scene) :
      EventHandler(scene)
  { 
      GLint viewport[4];
  ...
  */
  int MotionEventHandler::handle(SDL_Event* event){
      // note: the actual camera manipulations are performed in the 
      // MotionEventHandler::proceed() function
      ...
      if(event->type == SDL_VIDEORESIZE){
          ...
          if(this->cameraDrag){
              ...
              this->scene->rotate(-1 * event->motion.xrel*this->rotateHorizontal, Camera::HORIZONTAL);
              this->scene->rotate(event->motion.yrel*this->rotateVertical, Camera::VERTICAL);
          }
   ...
   */
  void MotionEventHandler::proceed(){
      if(this->resize){
      ...
      if(this->keydown){
          ...
          this->scene->rotate( this->rotateHorizontal, Camera::HORIZONTAL );
          this->scene->rotate( this->rotateVertical, Camera::VERTICAL );
      }
  }
  
  /// initialize new selection event handler
  SelectionEventHandler::SelectionEventHandler(Scene* scene) :
       EventHandler(scene)
      ,selectedObj(NULL)
  {}
  ...
  /**
      ...
      \returns 1 if event has been handled, else 0
  */
  int SelectionEventHandler::handle(SDL_Event* event){
      if (event->type == SDL_MOUSEBUTTONDOWN){
          ...
          switch (event->button.button){
              case SDL_BUTTON_LEFT: 
                  ...
                  if(this->selectedObj){
                      ...
                      SelectionMessage( this->selectedObj->id ).send();
                      return 1;
                  }
              default:
                  // do nothing
          ...
          switch (event->key.keysym.sym){
              ...
              case SDLK_UP:
                  ControllerMessage( MARK_Y_POS ).send();
                  return 1;
              case SDLK_DOWN:
                  ControllerMessage( MARK_Y_NEG ).send();
                  return 1;
              case SDLK_LEFT:
                  ControllerMessage( MARK_X_NEG ).send();
                  return 1;
              case SDLK_RIGHT:
                  ControllerMessage( MARK_X_POS ).send();
                  return 1;
              case SDLK_SPACE:
                  // select currently marked object
                  ControllerMessage( SELECT ).send();
                  return 1;
              default:
                  // do nothing
                  break;
  ...
  /// constructor
  ConsoleEventHandler::ConsoleEventHandler(Scene* scene) :
      EventHandler(scene)
  {
      this->active = false;
  }
  ...
  /**
      ...
      \returns 1 if event has been handled, else 0
  */
  int ConsoleEventHandler::handle(SDL_Event* event){
      if (event->type == SDL_KEYDOWN){
